 ...
 */
class PhoneBook {
    private val book1 = mutableMapOf<String, MutableSet<String>>()
     ...
     */
    fun addHuman(name: String): Boolean = if (name in book1) false else {
     ...
     * (во втором случае телефонная книга не должна меняться).
     */
    fun removeHuman(name: String): Boolean = if (name in book1) {
        book1.remove(name)
        true
    } else false
    ...
    fun addPhone(name: String, phone: String): Boolean {
        for (phones in book1.values) {
            if (phone in phones) return false
        }
        book1[name]?.add(phone) ?: return false
        return true
    }
    ...
    fun removePhone(name: String, phone: String): Boolean {
        return if (phone !in book1[name]!!)
            false
        else {
            book1[name]!!.remove(phone)
            true
        }
    }
     ...
     */
    fun phones(name: String): Set<String> = book1[name] ?: setOf()
    ...
    fun humanByPhone(phone: String): String? {
        for ((names, phones) in book1)
            if (phones.contains(phone)) return names
        return null
    }
     ...
     */
    override fun equals(other: Any?): Boolean = other is PhoneBook && book1 == other.book1
